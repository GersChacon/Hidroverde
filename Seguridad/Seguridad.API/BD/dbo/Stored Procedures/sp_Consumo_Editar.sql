...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @nueva_cantidad <= 0
            THROW 51023, 'La cantidad debe ser mayor a 0.', 1;
        ...
        -- Solo validar estado del ciclo cuando el consumo sí tiene ciclo asociado
        IF EXISTS (
            SELECT 1
            FROM dbo.Consumos co
            WHERE co.consumo_id = @consumo_id
              AND co.ciclo_id IS NOT NULL
        )
        BEGIN
            IF EXISTS (
                SELECT 1
                FROM dbo.Consumos co
                INNER JOIN dbo.Ciclos c
                    ON c.ciclo_id = co.ciclo_id
                INNER JOIN dbo.Estados_Ciclo ec
